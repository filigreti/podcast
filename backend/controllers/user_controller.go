...
func UserLogin(c echo.Context) error {
	...
	if user.IsVerified == nil || !*user.IsVerified {
		...
		return c.JSON(http.StatusUnauthorized, responses.UserResponse{Status: http.StatusUnauthorized, Message: "error", Data: &echo.Map{"data": "Email not verified. Please verify your email before logging in.", "code": "403"}})
	}
...
func sendGomail(path, token, email, username string) error {
	...
	data := EmailTemplateData{
		VerificationLink: fmt.Sprintf("http://localhost:3000/verification/%s", token),
		Username:         username,
	}
