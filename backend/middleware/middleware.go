...
func TokenMiddleware(next echo.HandlerFunc) echo.HandlerFunc {
    return func(c echo.Context) error {
        ...
        if !token.Valid {
            ...
            if refreshToken != "" {
                userID, err := controllers.ValidateAndRefreshToken(refreshToken)
                if err == nil {
                    // Generate a new access token and update the context with the user ID
                    newAccessToken, _, err := controllers.GenerateJwtToken(userID)
                    if err == nil {
                        c.Set("user", userID)
            ...
            // If refresh fails or there's no refresh token, return unauthorized
            if c.Get("user") == nil {
                return c.JSON(http.StatusUnauthorized, map[string]interface{}{"error": "Unauthorized"})
            }
        }
